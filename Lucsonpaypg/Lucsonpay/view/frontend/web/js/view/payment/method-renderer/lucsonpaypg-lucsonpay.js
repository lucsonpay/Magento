define(
    ...
    function ($, Component, placeOrderAction, selectPaymentMethodAction, customer, checkoutData, additionalValidators) {
        ...
        $('.errP').text('');
        var promo='';
        /*alert(window.checkoutConfig.payment.lucsonpay.hide_promo_field);
        alert($.trim(window.checkoutConfig.payment.lucsonpay.promoCode));
        alert($.trim(window.checkoutConfig.payment.lucsonpay.promoCode).length);
        alert(window.checkoutConfig.payment.lucsonpay.promoCode);*/
        if(window.checkoutConfig.payment.lucsonpay.hide_promo_field=='1'){
            if(window.checkoutConfig.payment.lucsonpay.promo_code_local_validation=='1' && ($.trim(window.checkoutConfig.payment.lucsonpay.promoCode).length=='0' || $.trim(window.checkoutConfig.payment.lucsonpay.promoCode)=='')){
        ...
        $(document).on('click','.promoApplyBtn',function(){
            // alert(window.checkoutConfig.payment.lucsonpay.promoCode);
            if(window.checkoutConfig.payment.lucsonpay.promo_code_local_validation=='1'){
                if($.trim($('.promoCodeField').val())==''){
                    $('.errP').text('Please enter your promo code.');
                }else{
                    var promoCode=window.checkoutConfig.payment.lucsonpay.promoCode;
        ...
        return Component.extend({
            defaults: {
                template: 'Lucsonpaypg_Lucsonpay/payment/lucsonpaypg'
            },
            ...
            afterPlaceOrder: function () {
                if(promo==''){
                    $.mage.redirect(window.checkoutConfig.payment.lucsonpay.redirectUrl);
                }else{
                    $.mage.redirect(window.checkoutConfig.payment.lucsonpay.redirectUrl+"?promo="+promo);
                }
            }
        });
    }
